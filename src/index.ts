...
import { aggregate, renderReport, type PriceConfig } from './report.js';
...
function hasActivity(events: readonly SessionEvent[]): boolean {
 ...
 */
export function writeReport(session: Session, config: ReportConfig): string | undefined {
	const events = session.events;
	if (!hasActivity(events)) return undefined;
 ...
 * reports on dispose.
 */
function apply(ctx: Context, config: ReportConfig) {
		...
		timers.set(
			...
			setTimeout(() => {
				...
				try {
					...
					if (file !== undefined) ctx.logger.info(`dsh-session-report: wrote ${file}`);
				} catch (error) {
